...
import { ServiceCategoriesData } from '../../../../../services/servicewomeyn/service-womeyn';
function CarouselCategoryService() {
  ...
  useEffect(() => {
    ServiceCategoriesData().then((res) => {
      setDatas(res?.data);
    }).catch((err) => {
      console.log(err);
    })
  ...
  const pushCatgorys = (data) => {
    history.push(`/servicecategory/${data}`)
  }
  return (
...
export default CarouselCategoryService;